...
1. **Netlist extraction:** A GDS file only contains geometry: polygons, layers, and cell placements. A netlist extractor walks every placed instance, matches it against the `sky130_fd_sc_hd` standard cell library, and traces metal/via layers to recover connectivity, producing a structural Verilog netlist of ~700 gates with no grouping or hierarchy.
2. **Module reconstruction:** Every unique standard cell (flip-flops, NAND/NOR/XOR gates, complex AND-OR-INVERT cells, clock buffers, etc.) was implemented by hand in Verilog so the design could be simulated in Quartus Prime / ModelSim, rather than generated automatically.
3. **Spatial clustering:** Place-and-route tools tend to keep related logic physically close together. Using the x/y coordinates embedded in the GDS file, a Python script grouped the flat netlist into 19 spatial clusters (proximity threshold 5.5), turning ~700 undifferentiated gates into something a human could actually reason about.
